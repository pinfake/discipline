...
Config.prototype._parseQueueConfig = function ( queueConfig, globalConfig )
{
    ...
    if( !queueConfig.minPendingJobs ) {
        if( !globalConfig.minPendingJobs ) {
            throw new SyntaxError('"minPendingJobs" property is neither defined on queue config nor in global config');
        } else {
            queueConfig.minPendingJobs = global.minPendingJobs;
        }
    }
    if( !queueConfig.maxConsumers ) {
        if( !globalConfig.maxConsumers ) {
            throw new SyntaxError('"maxConsumers" property is neither defined on queue config nor in global config');
        } else {
            queueConfig.maxConsumers = global.maxConsumers;
        }
    }
    if( !queueConfig.minConsumers ) {
        if( !globalConfig.minConsumers ) {
            throw new SyntaxError('"minConsumers" property is neither defined on queue config nor in global config');
        } else {
            queueConfig.minConsumers = global.minConsumers;
        }
    }
};
